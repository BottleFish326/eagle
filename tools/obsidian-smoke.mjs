...
if (command === "setup") {
  await setup(
    requiredPath(options, "vault"),
    requiredPath(options, "materials"),
  );
  ...
  throw new Error(
    "usage: node tools/obsidian-smoke.mjs setup|clean --vault PATH --materials PATH",
  );
}
...
async function setup(vaultPath, materialsPath) {
  ...
  const pluginTarget = path.join(
    vaultPath,
    ".obsidian/plugins/material-bridge",
  );
  ...
  await Promise.all([
    copyFile(
      path.join(pluginSource, "main.js"),
      path.join(pluginTarget, "main.js"),
    ),
    copyFile(
      path.join(pluginSource, "manifest.json"),
      path.join(pluginTarget, "manifest.json"),
    ),
  ]);
