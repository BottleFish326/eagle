...
try {
  ...
  if (!stats.isFile())
    throw new Error("phase 2 exit evidence is not a regular file");
  if (stats.size > 1024 * 1024)
    throw new Error("phase 2 exit evidence exceeds 1 MiB");
  const value = JSON.parse(await readFile(evidencePath, "utf8"));
  const inspection = inspectPhase2ExitGatesReceipt(value);
...
function parseArguments(args) {
  if (args.length > 1)
    throw new Error(
      "usage: node tools/inspect-phase-2-exit-evidence.mjs [receipt.json]",
    );
  return path.resolve(
    args[0] ??
      path.join(
...
function assertNode24() {
  if (Number.parseInt(process.versions.node.split(".", 1)[0], 10) !== 24)
    throw new Error(
      `phase 2 exit evidence inspection requires Node.js 24.x, received ${process.version}`,
    );
}
