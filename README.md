...
`npm run status:p2-exit` 是只读阶段编排器：它检查 Git 状态、A11 final/partial、A12 原始归档与 hosted 收据、外部汇总、本地故障收据、数据安全审计和最终退出收据，必要时从原始字节重放外部门禁。命令不会构建、推送、触发 workflow 或写证据；输出唯一当前阶段和下一动作，只有发现矛盾、失败或损坏的不可覆盖证据时才返回非零。
