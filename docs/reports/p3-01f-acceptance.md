...
格式清单现包含 42 个源文件、1,468,255 字节源内容和 32,975 字节引用计数。新增 10 项均
覆盖 Windows、macOS 与 Linux 的 `core-only` 期望；普通与主动内容 PDF 固定为 2 页、
612 × 792。PNG 伪装夹具按内容识别为 PNG，并使用内置 raster provider 重编码后
SHA-256 为 `d6186dc0…` 的 16 × 16 参考图；反向伪装按内容识别为 PDF。扫描测试逐个复核
所有源文件字节不变。
