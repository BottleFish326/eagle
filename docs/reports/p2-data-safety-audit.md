...
本门禁以 [`docs/defects.json`](../defects.json) 为唯一阶段缺陷登记输入。仓库中的初始状态故意为 `draft`；即使没有 open finding，draft 也不能满足退出条件。A11/A12 外部门禁与 A04/A10 候选崩溃收据完成后，已逐项复核完整 CI、81 项工具测试、九份控制报告、失败现场、四项已修复发现和所有已知限制。登记册现为 `reviewed`，审核时间晚于两份正式上游证据。
...
四项条目均有复现或修复证据，正式重放未再发现新的 P0/P1/P2/P3 问题；最终计数为 open 各级均 0、resolved P1 两项、P2/P3 各一项。不得为了维持零 open 计数而省略后续发现。
...
登记册、两个 Schema、纯分析器、离线检查器、不可覆盖生成器、P2 状态转换和最终历史重放均已实现。仓库全部 15 个 Draft 2020-12 Schema 由锁定版本验证器严格编译；数据安全收据 Schema 固定九份报告与十项控制的精确顺序，并具有生成正例、重复报告、控制重排和 open P1 篡改负例。登记册包含 resolved P1 两项、P2/P3 各一项，open 各级均为零，并已在 `2026-08-20T12:37:33.000Z` 完成 reviewed。正式收据绑定提交 `11db4c178b824684de6616976c61d4590f5139df`，为 `accepted=true`、`failures=[]`，SHA-256 为 `a87c0ee715eeab797b546a9cf5d5d91b8dfe94992341780180fa944062292d76`；离线重放、JSON Schema 与敏感信息审计均通过。
