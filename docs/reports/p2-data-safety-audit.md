...
登记册、两个 Schema、纯分析器、离线检查器、不可覆盖生成器、P2 状态转换和最终历史重放均已实现。仓库全部 15 个 Draft 2020-12 Schema 现在由锁定版本验证器严格编译；数据安全 accepted 收据同时具有生成正例和 open P1 篡改负例。正式 A11/A12 与本地候选故障证据尚未齐备，因此登记册保持 `draft`，不得生成 accepted 数据安全收据，也不得把“P0/P1 为零”标记为 Pass。
