...
| 阶段 2 退出验收 | Draft / Not accepted | P2-A01 至 P2-A10 已汇总；只读状态编排器覆盖长稳、托管、外部汇总、本地故障、数据安全和最终收据；外部、数据安全与最终 accepted-only 收据均有独立离线自检；最终验证器重放 A11/A12、历史缺陷登记及九份安全控制报告，并核对源码漂移、证据归属与五段 commit 顺序；P2-A11 正式任务运行中，P2-A12 等待托管结果，缺陷登记保持 draft；见 `reports/phase-2-acceptance.md` |
