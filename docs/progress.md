...
| P2-08 平台与文件系统兼容 | Acceptance pending | macOS 原生 10 项通过；三平台证据器精确要求 10/12/9 项，matrix 汇总器绑定同一 commit/run/attempt；手动触发与只读预检已就绪，指定 run/attempt 采集器会复核托管元数据、下载并重放四个 artifact 后原子归档；P2-A12 首次远程运行仍待完成；见 `reports/p2-08-acceptance.md` |
